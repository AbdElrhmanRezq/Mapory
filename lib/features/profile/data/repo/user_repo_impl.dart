...
class UserRepoImpl implements UserRepo {
  ...
  @override
  Future<List<PhotoModel>> getPhotosByMemoryId({
    ...
    return Future(() async {
      try {
        ...
        final List<MemoryModel> memories = records
            .map((memoryData) => MemoryModel.fromMap(memoryData))
            .toList();
        memories.forEach((memory) async {
          memory.photos = await getPhotosByMemoryId(memoryId: memory.memoryId);
        });
